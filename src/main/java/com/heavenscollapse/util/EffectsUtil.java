...
public class EffectsUtil {
    ...
    /** Rays of the outward Sculk Soul burst (evenly spaced around a circle). */
    private static final int WARDEN_BURST_RAYS = 16;

    /** How many expanding steps the burst takes before it finishes. */
    private static final int WARDEN_BURST_STEPS = 6;

    /** Final radius, in blocks, the burst reaches. */
    private static final double WARDEN_BURST_MAX_RADIUS = 3.5;
    ...
    /**
     * Animates rings of Sculk Soul particles - the glowing teal wisps that
     * drift off Sculk Catalysts and the Warden itself - shooting outward
     * from the impact point over a handful of ticks, like a shockwave.
     * Runs as a short-lived repeating task rather than a single instant
     * spawn so the particles visibly travel outward instead of just
     * appearing already spread out.
     */
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                for (int i = 0; i < WARDEN_BURST_RAYS; i++) {
                    ...
                    world.spawnParticle(Particle.SCULK_SOUL, point, 1, 0, 0, 0, 0);
                }
