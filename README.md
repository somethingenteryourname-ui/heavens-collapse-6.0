...
- **Sounds**: lightning thunder + impact, a heavy mace smash, a beacon
  chime for the "divine" feel, and the Warden's sonic boom layered in as
  a distinct, instantly recognizable signature sound - all vanilla, so no
  resource pack is required.
- **Particles**: a flash, END_ROD light rays, electric sparks, cloud
  burst, and a Sonic Boom particle (which carries its own built-in
  expanding-ring animation) all centered on the target, plus a spark
  trail connecting the attacker to the target so the strike reads as
  coming from them.
- **Warden burst**: rings of Sculk Soul particles (the glowing teal wisps
  from Sculk Catalysts / the Warden) animate outward from the impact
  point over about six ticks, like a shockwave - implemented as a short
  repeating task rather than a single spawn, so the particles visibly
  travel outward instead of just appearing already spread out.
